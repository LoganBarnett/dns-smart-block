...
The system uses ~dnsdist~ as the DNS server.  ~dnsdist~ is a dynamic DNS server
that allows adding blocked domains via API without restarts or reloads.  This
makes it ideal for real-time, LLM-driven blocking decisions.

DNS Smart Block consists of two main components:

1. *Log Processor* - Watches ~dnsdist~ logs and enqueues domains that appear in
   DNS requests.  When a DNS lookup occurs, it's written to the log, and the log
   processor adds it to the work queue.

2. *Worker* - Consumes domains from the queue, fetches data from each domain
   (HTTP GET with redirect following), and feeds the content to an LLM.  The LLM
   classifies the site based on your interested categories.  If the site
   matches, the worker updates ~dnsdist~'s block list via its API.
...
parallel_flows: {
  grid-gap: 4
  dnsdist_to_upstream: "dnsdist forwards to upstream"

  dnsdist_log: "dnsdist logs request"
  log_processor: "log processor enqueues domain"
  worker_http: "worker fetches HTTP"
  worker_llm: "worker queries LLM"
  worker_dnsdist_update: "worker updates dnsdist"

  dnsdist_log -> log_processor -> worker_http -> worker_llm -> worker_dnsdist_update
}

initial_query --> dnsdist_handle
dnsdist_handle -> parallel_flows.dnsdist_to_upstream
...
#+begin_example
 ...
 │                          ▼               ▼                  │
 │    ┌───────────────────────────┌─────────────────────┐      │
 │    │dnsdist forwards to upstrea│dnsdist logs request │      │
 │    │                           │ │                   │      │
 │    └───────────────────────────└─────────────────────┘      │
 │                                          │                  │
 │                                          ▼                  │
...
This section covers how to deploy and configure DNS Smart Block on your host system.
      ...
      modules = [
        dns-smart-block.nixosModules.default
        {
          services.dns-smart-block = {
            ...
            # Watch dnsdist logs via journalctl
...
- *Built-in NATS server* - Dedicated instance that won't conflict with
  other NATS services
- *Systemd services* - Automatically configured log-processor and
  queue-processor
...
- *Flexible log sources* - Support for both journalctl and file-based
  log watching
- *Secret management* - Compatible with agenix, sops-nix for API keys

*** NATS Isolation

The built-in NATS server is isolated to avoid conflicts:

- Configurable port (default: 4222)
- Separate data directory: =/var/lib/dns-smart-block-nats=
- Unique subject: =dns.smart-block.domains=
- Can be disabled to use external NATS

*** Permissions

When using =cmd:journalctl -f -u dnsdist=:
- Service automatically gets =systemd-journal= supplementary group
- Can read dnsdist logs without root privileges
- Works with dnsdist's journald integration

When using file-based log source:
- Service gets read-only access to specified file path
...
services.dns-smart-block = {
  ...
  nats.subject = "dns.smart-block.domains";

  # Log processor
  logProcessor.enable = true;
  logProcessor.logSource = "cmd:journalctl -f -u dnsdist";
  logProcessor.skipDnsdistCheck = false;

  # Queue processor
  queueProcessor.enable = true;
  queueProcessor.minConfidence = 0.8;    # 0.0 to 1.0
  queueProcessor.httpTimeoutSec = 10;
  queueProcessor.httpMaxKb = 100;

  # Ollama LLM
  ollama.url = "http://localhost:11434";
  ollama.model = "llama2";

  # dnsdist API (optional)
...
- =nix/MODULE.org= - Comprehensive module documentation
- =nix/module-example.nix= - Working example configuration
- =nix/SUMMARY.org= - Quick reference guide
...
** TODO [#C] Implement two Rust projects: A log-processor and a worker
...
#+begin_src sh :results output :exports both
# IGN apparently doesn't like anyone just sending a curl at them.
# The nerve!  Apparently they haven't heard of Roko's Basilisk...
curl \
  --location \
  --silent \
  --show-error \
...
#+RESULTS:
: IGN is your #1 destination for all video game news, expert reviews,
: and walkthroughs.
: IGN is your #1 destination for all video game news, expert reviews,
: and walkthroughs.


Alright, I asked ChatGPT 5.1 nicely and it gave me a wonderfully detailed
prompt to use:
...
A site SHOULD be classified as a gaming site ("is_gaming_site": true) if
its PRIMARY purpose is one or more of:
...
2.  Being a platform for playing or distributing games:
   ...
   - Examples that SHOULD be TRUE:
     - scratch.mit.edu (learn-to-code via interactive projects and
       games)
     - newgrounds.com (hosts lots of browser games and game-like
       content)

3.  Being a platform specifically aimed at game creation or game mods:
   - Game engines, asset stores, or tools whose primary branding is
     around game development.

4.  Being a site primarily about game-related community or esports:
   - Forums, wikis, or news sites dedicated mainly to specific games or
     gaming communities.

A site SHOULD NOT be classified as a gaming site ("is_gaming_site":
false) if:

1.  It is a general-purpose social network, chat platform, or content
   host whose main purpose is NOT specifically gaming, even if it has a
   lot of game content.
...
2.  It is obviously about something else:
   - News, banking, education, general tech, streaming for all topics,
     etc., with gaming only as a minor or incidental category.

3.  The provided information is too vague or generic to clearly
   indicate a primary gaming focus.  In that case, be conservative and
   answer false with a lower confidence.

INPUT FORMAT

You will receive a single JSON object describing the site.  It will
always include:
- "domain": the domain name as a string.
- At least one free-text field with descriptive text, for example:
  - "summary", "description", "title", "meta_description",
    "og_description", etc.

The JSON may include MORE fields in the future.  Treat all string fields
as possible clues about the site's purpose.  Ignore fields you do not
understand; do NOT fail because of unknown fields.
...
- Do NOT include any extra text, comments, or explanations outside the
  JSON.
...
{
  "domain": "ign.com",
  "summary": "IGN is your #1 destination for all video game news,\
 expert reviews, and walkthroughs."
}
Output:
...
Input:
{
  ...
  "summary": "Scratch is a block-based visual programming language\
 and online community for creating and sharing interactive media such as\
...
Input:
{
  ...
  "summary": "Newgrounds is an online entertainment website and\
 community featuring user-made games, movies, art and audio."
}
Output:
...
{
  "domain": "reddit.com",
  "summary": "Reddit is a network of communities where people can dive\
...
Input:
{
  ...
  "summary": "Khan Academy is a nonprofit organization with the\
...
domain='ign.com'
description='IGN is your #1 destination for all video game news, \
expert reviews, and walkthroughs.'
sed \
  "s/{{INPUT_JSON}}/{ \"domain\": \"${domain}\", \
...
jq \
  --null-input \
  '{ model: $ENV.model, prompt: $ENV.full_prompt, format: "json", \
stream: false}' \
    2>&1 \
  | curl \
    --location \
