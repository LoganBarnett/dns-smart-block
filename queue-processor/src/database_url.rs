...
pub fn construct_database_url(
  ...
  if let Some(password_path) = password_file {
    ...
    let password = fs::read_to_string(password_path)?.trim().to_string();

    // Parse the base URL and inject password
    let mut url = Url::parse(base_url)?;
    url.set_password(Some(&password)).map_err(|_| {
      DatabaseUrlError::UrlParseError(url::ParseError::InvalidDomainCharacter)
    })?;
...
mod tests {
  ...
  fn test_construct_url_with_password_file() {
    ...
    let result =
      construct_database_url(base_url, Some(temp_file.path())).unwrap();
