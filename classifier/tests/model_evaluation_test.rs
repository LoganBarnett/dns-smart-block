...
use dns_smart_block_classifier::{
  classify_with_llm, web_classify::SiteMetadata,
};
use std::env;
...
async fn test_model_classification(
  ...
  let result =
    classify_with_llm(metadata, &ollama_url, model, GAMING_PROMPT_TEMPLATE)
      .await?;
...
async fn test_phi3_mini_gaming_site() {
  test_model_classification("phi3:mini", &create_gaming_site_metadata(), true)
    .await
...
async fn test_custom_model_gaming_site() {
  ...
  test_model_classification(&model, &create_gaming_site_metadata(), true)
    .await
...
async fn test_custom_model_non_gaming_site() {
  ...
  test_model_classification(&model, &create_non_gaming_site_metadata(), false)
    .await
    .expect("Custom model test failed");
}
