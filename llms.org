...
- Use full punctuation for all sentences, including list items.
- Use two spaces between the end of a sentence and the start of the next.
- Wrap prose to 80 columns.
...
State: sqlx + PostgreSQL for dedup, backoff, last-seen timestamps
...
c) not seen recently (PostgreSQL state with TTL)
d)
...
Else: record in PostgreSQL with decision + confidence; optionally enqueue for manual review.
...
Always use ~nix develop~ to get access to ~cargo~ and other build tools rather
than pulling them from global ~nixpkgs~:

#+BEGIN_SRC bash
cd /path/to/dns-smart-block && nix develop --command cargo build
#+END_SRC
