...
use dns_smart_block_worker::{
    classify_with_llm, web_classify::{extract_metadata, fetch_domain},
};
...
async fn process_domain(
    ...
    let (html, status) = match fetch_domain(domain, args.http_timeout_sec, args.http_max_kb).await {
        Ok(result) => result,
        Err(e) => {
    ...
    if classification.is_matching_site && classification.confidence >= args.min_confidence {
        info!(
        ...
        if let Some(ref dnsdist_url) = args.dnsdist_api_url {
            ...
            info!("No dnsdist API configured, skipping block action for {}", domain);
        }
    } else {
        info!(
    ...
    while let Some(message) = subscriber.next().await {
        ...
        match serde_json::from_slice::<DomainMessage>(&payload) {
            Ok(domain_msg) => {
                ...
                // Process the domain
                if let Err(e) = process_domain(&domain_msg.domain, &args, &prompt_template).await {
